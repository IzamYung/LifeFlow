    ...
    showAddEvent(onSaveSuccess) {
        ...
        Modal.open('Add Schedule Event', html, (container) => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                const recurrence = document.getElementById('ev-recur').value;
                ...
                try {
                    await API.execute("INSERT INTO schedule (title, type, location, start_time, end_time, description, recurrence, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        [title, type, location ? location : null, start_time, end_time, description ? description : null, recurrence, color]);

                    // Queue alerts based on event type with a time safety check
        ...
        Modal.open('Add Assignment', html, (container) => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    // 2. Schedule a 5-day reminder ONLY if the due date is >7 days away
